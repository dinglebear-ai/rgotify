...
#[derive(Clone)]
pub struct GotifyRmcpServer {
    state: AppState,
}

pub fn rmcp_server(state: AppState) -> GotifyRmcpServer {
    GotifyRmcpServer { state }
}

impl ServerHandler for GotifyRmcpServer {
    // ── tools ─────────────────────────────────────────────────────────────────
...
) -> StreamableHttpService<GotifyRmcpServer, LocalSessionManager> {
...
const SCHEMA_RESOURCE_URI: &str = "gotify://schema/mcp-tool";

fn schema_resource() -> Resource {
    Resource::new(
        RawResource::new(SCHEMA_RESOURCE_URI, "gotify tool schema")
            .with_description("JSON schema for the gotify MCP tool and its action-based parameters")
            .with_mime_type("application/json"),
        None,
    )
}
...
fn check_scope(auth: &AuthContext, required_scope: &str, action: &str) -> Result<(), ErrorData> {
    let satisfied = auth
        .scopes
        .iter()
        .any(|s| s == required_scope || (required_scope == READ_SCOPE && s == "gotify:write"));
    if satisfied {
        return Ok(());
    }
    tracing::warn!(
...
fn is_validation_error(error: &anyhow::Error) -> bool {
    ...
    s.contains(" is required")
        || s.contains("must be an integer")
        || s.contains("unknown gotify action")
}
