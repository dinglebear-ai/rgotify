...
const APPDATA_ENV: &str = "GOTIFY_MCP_HOME";
...
fn apply_plugin_options() {
    // CLAUDE_PLUGIN_DATA is intentionally NOT mapped to GOTIFY_MCP_HOME: appdata
    // resolves to the canonical ~/.gotify/ (the same place the binary loads .env
    // from via config::load_dotenv), not the plugin's sandboxed data dir. An
    // explicit GOTIFY_MCP_HOME override is still honored by appdata_dir().

    // CLAUDE_PLUGIN_OPTION_<OPT> -> <GOTIFY_ENVVAR>
    let map = [
        ("CLAUDE_PLUGIN_OPTION_API_TOKEN", "GOTIFY_MCP_TOKEN"),
        ("CLAUDE_PLUGIN_OPTION_NO_AUTH", "GOTIFY_MCP_NO_AUTH"),
        ("CLAUDE_PLUGIN_OPTION_MCP_HOST", "GOTIFY_MCP_HOST"),
        ("CLAUDE_PLUGIN_OPTION_MCP_PORT", "GOTIFY_MCP_PORT"),
        ("CLAUDE_PLUGIN_OPTION_AUTH_MODE", "GOTIFY_MCP_AUTH_MODE"),
        ("CLAUDE_PLUGIN_OPTION_PUBLIC_URL", "GOTIFY_MCP_PUBLIC_URL"),
        (
            "CLAUDE_PLUGIN_OPTION_GOOGLE_CLIENT_ID",
            "GOTIFY_MCP_GOOGLE_CLIENT_ID",
        ),
        (
            "CLAUDE_PLUGIN_OPTION_GOOGLE_CLIENT_SECRET",
            "GOTIFY_MCP_GOOGLE_CLIENT_SECRET",
        ),
        (
...
fn repair_report() -> Result<SetupReport> {
    ...
    if !env_path.exists() {
        std::fs::write(&env_path, b"# Managed by gotify setup repair.\n")?;
    }
    Ok(SetupReport {
        mode: "repair",
        ..check_report()
...
fn port_check() -> SetupCheck {
    let port = setup_port("GOTIFY_MCP_PORT", 40020);
...
fn appdata_dir() -> PathBuf {
    ...
    gotify_mcp::config::default_data_dir()
}
