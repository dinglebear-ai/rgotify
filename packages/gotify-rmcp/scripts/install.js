...
function log(message) {
  process.stderr.write(`gotify-rmcp: ${message}\n`);
}
...
async function main() {
  if (process.env.GOTIFY_RMCP_SKIP_DOWNLOAD === "1") {
    log("skipping binary download because GOTIFY_RMCP_SKIP_DOWNLOAD=1");
    return;
  }
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "gotify-rmcp-install-"));
