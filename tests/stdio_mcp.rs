...
async fn stdio_client(
    ...
    let (transport, stderr) = TokioChildProcess::builder(Command::new(binary).configure(|cmd| {
        cmd.arg("mcp")
            ...
            .env("GOTIFY_CLIENT_TOKEN", "test-client")
            .env("GOTIFY_APP_TOKEN", "test-app")
            .env("GOTIFY_MCP_NO_AUTH", "true")
            .env("RUST_LOG", "warn")
            .env_remove("GOTIFY_MCP_TOKEN");
    }))
    .stderr(Stdio::piped())
    .spawn()?;
...
async fn stdio_child_process_lists_tools_and_calls_queries() {
    ...
    assert_eq!(names, vec!["gotify"]);

    let status = service
        .call_tool(
            CallToolRequestParams::new("gotify")
    ...
    assert!(help["help"].as_str().unwrap_or("").contains("gotify"));
    ...
    if let Some(mut stderr) = stderr {
        ...
        assert!(
            !logs.contains("gotify listener") && !logs.contains("MCP server listening"),
