...
fn status_call() -> serde_json::Value {
    serde_json::json!({ "name": "gotify", "arguments": { "action": "status" } })
}

// ── tests ─────────────────────────────────────────────────────────────────────

/// Valid JWT with `gotify:read` -> `tools/call action=status` succeeds (200).
#[tokio::test]
async fn valid_jwt_with_read_scope_allows_status() {
    ...
    let token = auth_state
        .signing_keys
        .issue_access_token(&make_claims(&auth_state, "gotify:read", 60))
        .unwrap();
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "valid JWT with gotify:read should succeed"
    );
    assert!(
...
/// `gotify:write` implicitly satisfies `gotify:read` — status must succeed.
#[tokio::test]
async fn valid_jwt_with_write_scope_satisfies_read() {
    let dir = TempDir::new().unwrap();
    let (state, auth_state) = testing::oauth_state_with_auth_state(dir.path()).await;
    let token = auth_state
        .signing_keys
        .issue_access_token(&make_claims(&auth_state, "gotify:write", 60))
        .unwrap();
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "gotify:write must implicitly satisfy gotify:read"
    );
}
...
async fn expired_jwt_returns_401() {
    ...
    let token = auth_state
        .signing_keys
        .issue_access_token(&make_claims(&auth_state, "gotify:read", -120))
        .unwrap();
...
async fn jwt_with_wrong_issuer_returns_401() {
    let dir = TempDir::new().unwrap();
    let (state, auth_state) = testing::oauth_state_with_auth_state(dir.path()).await;
    let mut bad_claims = make_claims(&auth_state, "gotify:read", 60);
...
/// `tools/list` with valid JWT -> 200 and gotify tool present.
...
async fn tools_list_succeeds_with_valid_jwt() {
    ...
    assert!(
        tools.is_some_and(|t| t.iter().any(|tool| tool["name"] == "gotify")),
        "tools/list must return the gotify tool"
    );
}
...
async fn jwt_signed_with_wrong_key_returns_401() {
    ...
    let claims = make_claims(&auth_state, "gotify:read", 60); // valid iss/aud for server
