#!/bin/sh
# entrypoint.sh — Docker container entrypoint for gotify-mcp
...
missing_vars=""
for var in GOTIFY_URL GOTIFY_CLIENT_TOKEN; do
    eval "val=\${${var}:-}"
    if [ -z "${val}" ]; then
...
# GOTIFY_APP_TOKEN is needed for sending — warn but do not fail.
if [ -z "${GOTIFY_APP_TOKEN:-}" ]; then
    echo "WARN: GOTIFY_APP_TOKEN is not set — send_message will be unavailable" >&2
fi

# ── 3. Data directory ─────────────────────────────────────────────────────────
mkdir -p "${DATA_DIR}/logs"

# Fix ownership — volume may have been created by another process or UID.
...
[ -n "${GOTIFY_MCP_PORT:-}" ] && echo "[entrypoint] MCP port: ${GOTIFY_MCP_PORT}"
[ -n "${GOTIFY_MCP_HOST:-}" ] && echo "[entrypoint] MCP host: ${GOTIFY_MCP_HOST}"
